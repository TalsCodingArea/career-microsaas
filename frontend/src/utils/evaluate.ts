/**
 * Client-side evaluation scoring logic.
 *
 * Mirrors the backend evaluationService algorithm but runs entirely in the
 * browser using mock/static data (no DB queries).
 *
 * v1 algorithm:
...
function pickContacts(careerPath: string): NetworkingContact[] {
  const relevant = MOCK_CONTACTS
    .filter(c => c.careerPathTags.includes(careerPath))
    .sort((a, b) => b.relevanceScore - a.relevanceScore)
    .slice(0, 3)
  if (relevant.length > 0) return relevant
  return MOCK_CONTACTS.slice(0, 2)
}

export function runEvaluation(answers: Answer[]): EvaluationResult {
  ...
  const marketSnapshot: MarketSnapshot = {
    ...
    openPositions: mockFallback.openPositions ?? 2000,
    avgSalaryMin: mockFallback.avgSalaryMin ?? 15000,
    avgSalaryMax: mockFallback.avgSalaryMax ?? 28000,
    currency: 'ILS',
    demandLevel: mockFallback.demandLevel ?? 'medium',
  }
