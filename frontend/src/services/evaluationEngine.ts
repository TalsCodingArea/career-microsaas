/**
 * Client-side evaluation engine — mirrors backend/src/services/evaluationService.ts
 * but runs entirely in the browser using mock data (no DB or network calls).
 */

import type { Answer, EvaluationResult, MarketSnapshot, NetworkingContact, PinnedRepo, SkillItem } from '../types/QuestionnaireTypes.ts'
...
function deriveTimeline(score: number): { min: number; max: number; label: string } {
...
export function runEvaluationClientSide(answers: Answer[]): EvaluationResult {
  ...
  const mockData = MOCK_MARKET[targetRole] ?? MOCK_MARKET['default']!

  const marketSnapshot: MarketSnapshot = {
    careerPath: targetRole,
    openPositions: mockData.openPositions ?? 2000,
    avgSalaryMin: mockData.avgSalaryMin ?? 15000,
    avgSalaryMax: mockData.avgSalaryMax ?? 28000,
    currency: 'ILS',
    demandLevel: mockData.demandLevel ?? 'medium',
  }
  ...
  const score = Math.min(1, rawScore / 100)

  const timeline = deriveTimeline(rawScore)
  const tips = buildTips(answers, rawScore, marketSnapshot.demandLevel)

  const contacts = MOCK_CONTACTS
    .filter(c => c.careerPathTags.includes(targetRole))
    .sort((a, b) => b.relevanceScore - a.relevanceScore)
    .slice(0, 3)

  const finalContacts = contacts.length > 0 ? contacts : MOCK_CONTACTS.slice(0, 2)
  ...
  return {
    ...
    marketSnapshot,
    tips,
    networkingContacts: finalContacts,
